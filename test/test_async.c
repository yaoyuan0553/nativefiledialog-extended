...
//    params.defaultPath = "/home/yuan/byte_wine/wine";
    ...
    // show the dialog
    nfdresult_t result = NFD_SaveDialogWinAsync(&params);
    if (result == NFD_OKAY) {
        puts("Success!");
        while (!NFD_HasAsyncOpCompleted()) {
            usleep(1000);
        }
        NfdDialogResponse response;
        if ((result = NFD_GetAsyncOpResult(&response)) == NFD_OKAY) {
            puts(*response.outPath);
            // remember to free the memory (since NFD_OKAY is returned)
            NFD_FreePath(*response.outPath);
        }
